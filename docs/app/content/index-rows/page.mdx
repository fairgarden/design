...
Event rows carry a mono date key, `--role-rule` rules and, from 768 px of container, a column rule between the date and the title. `IndexRowDisclosure` makes the title the trigger of a Collapsible: collapsed titles are muted, open ones grow and take the full ink, and the disclosure glyph turns inward. Past events say "Past" in words; they are never dimmed. Rows print expanded with the date column at the left.
