...
Pass `pmtiles={{ url, center, zoom }}` (plus `minZoom`, `maxZoom`, `ratio`, `lang`, `glyphs`, `workerUrl` and `attribution` as needed). The archive must use the Protomaps basemap schema v4 or later and be served with HTTP range requests and CORS. `kind="location"` draws the `--radius-3-25` frame with a `--border-size-2` `--primary12` edge and always carries a mixed-case text address. `routes` draws `line-dashed` routes in `--role-accent` (alternates in `--primary12`) over the roads and under the labels.
