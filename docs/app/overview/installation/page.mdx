...
The rest comes with the package: the fonts (Fontsource), Recharts for [Chart](../../data/chart/page.mdx), and the Material Symbols Rounded icons, generated as per-icon SVG paths, so there is no icon font to load.

## Maps

[Map](../../data/map/page.mdx)'s vector base draws OpenStreetMap data from a Protomaps PMTiles archive with MapLibre GL JS. Its three libraries are optional peer dependencies: install them only in apps that pass `pmtiles` to a `Map`.
...
The page root is a `paper` band. Wrap regions in a [Ground](../../foundations/ground/page.mdx) to change it.
