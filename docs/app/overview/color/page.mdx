...
1. the prop;
2. the nearest [Ground](../../components/ground/page.mdx) scope's defaults, inherited as custom properties;
...
- **Contained buttons lighten.** A solid [Button](../../components/button/page.mdx)'s fill moves to `--role-action-hover`, and a contained selection fill to `--role-select-hover`. Label and edge stay, with no underline.
- **Outline buttons tint** to `--role-soft-hover`; where soft fills drop, the label underlines instead.
- **Chevron links change color.** A `standalone` [Link](../../components/link/page.mdx) and a `text` Button take `--role-link-hover`, and the glyph swaps to its next weight.
