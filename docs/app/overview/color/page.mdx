...
Hover follows one model [D181]: it changes a fill or an ink, never an edge, a transform or a shadow. It runs for `--fgd-duration-quick` with `--ease-out-3`, is instant under reduced motion, and never applies to a disabled control.
...
Wherever `--role-link-hover` is `--primary12`, an ink-only hover also underlines (`--fgd-link-hover-line`); where soft fills drop, `--fgd-soft-hover-line` underlines the outline button's label. Under forced colors, hover adds an underline to links and text and an outline to buttons.
